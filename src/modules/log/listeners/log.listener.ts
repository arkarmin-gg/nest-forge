...
import { ACTIVITY_LOG_EVENT, ActivityLogEvent } from '../events/activity-log.event';
...
export class LogListener {
  ...
  async handleActivityLog(event: ActivityLogEvent): Promise<void> {
    await this.activityLogService.create(event).catch((err: unknown) =>
  ...
  async handleAuditLog(event: AuditLogEvent): Promise<void> {
    await this.auditLogService.create(event).catch((err: unknown) =>
