...
| **STD-16** | `npm run typecheck`, `npm run build`, and `npm run knip` introduce no _new_ errors/findings versus the pre-change baseline (the repo has pre-existing knip findings not yet cleaned up — flag only findings the diff introduces, not the existing baseline).                  | mechanical            | high     | §22                                                   |
